export const services = [
    {
        name: 'Enterprise Software Development',
        heading: 'Captivate Your Audience.',
        p:
            "Whether we're building a web app, desktop driven system or mobile application, we excel at creating user-friendly interfacing.  Enjoy our distinct navigation and scroll techniques that will keep your customers naturally engaged, increase conversion, and drive revenue.",
        li: [
            'Full Google Suite Integration',
            'Full Stack Development',
            'Web App, Desktop, and Mobile Application',
            'Full Content Strategy',
            '2D and 3D Animation',
            'Scalable Systems',
            'UI/UX Design',
        ],
    },
    {
        name: 'Public Relations & Crisis Management',
        heading: 'Refine Your Identity.',
        p:
            'Develop brand identities that reflect your company’s values and resonate with your customers.',
        li: [
            'Digital Communications',
            'Strategic Rebranding',
            'Reputation Appraisal',
            'Press Release and SEO',
            'Cause Marketing',
        ],
    },
    {
        name: 'Business Consulting',
        heading: 'Maximize Your Operation',
        p: 'We are thinkers, creators, and do-ers.  We push ourselves to seek our new and creative solutions in all aspects of business. We know marketing strategies founded in the creative leave the longest lasting impact.',
        li: [
            'Go to Market Plans',
            'Lean Six Sigma',
            'Kaizen',
            'System Auditing',
            'Enterprise Resource Planning',
            'Application Integration',
        ],
    },
    {
        name: 'Website Development/PPC/SEO',
        heading: 'Show Your Best Face',
        p: 'Although we offer the full suite we understand that you may need your services a-la-carte. You can rest assured our mobile-first, responsive interfaces, distinct navigation and powerful marketing will keep your customers naturally engaged, increase conversion, and drive revenue.',
        li: [
            'Responsive UI',
            'Mobile-first Design',
            'Full Google Suite Integration',
            'PPC Campaigns',
            'Platinum Level Hosting',
            'Full Content Strategy',
            '2D and 3D Animation',
        ],
    },
];

export const tech = [
    {
        name: '.Net',
        img: 'img/brands/ico-01.png',
    },
    {
        name: 'PHP',
        img: 'img/brands/ico-02.png',
    },
    {
        name: 'HTML/CSS',
        img: 'img/brands/ico-03.png',
    },
    {
        name: 'Webflow',
        img: 'img/brands/ico-04.png',
    },
    {
        name: 'Javascript',
        img: 'img/brands/ico-05.png',
    },
    {
        name: 'Node.js',
        img: 'img/brands/ico-06.png',
    },
    {
        name: 'Node.js',
        img: 'img/brands/ico-08.png',
    },
    {
        name: 'Express.js',
        img: 'img/brands/ico-09.png',
    },
    {
        name: 'Ruby on Rails',
        img: 'img/brands/ico-10.png',
    },
    {
        name: 'Angular',
        img: 'img/brands/ico-11.png',
    },
    {
        name: 'Svelte',
        img: 'img/brands/ico-12.png',
    },
    {
        name: 'React',
        img: 'img/brands/ico-13.png',
    },
];
