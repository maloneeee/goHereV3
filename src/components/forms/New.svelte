<script>
  ...
  async function sendContactMessage() {
    try {
      ...
      if(form.website <= 3){
        err = "Please put your current website. If you don't have one please put 'None'"
        numE++
      }
...
<div class="form_holder">
    ...
    <form action="" on:submit|preventDefault={sendContactMessage} out:fade='{{delay: 0, duration: 500}}'>
        <div class="form_body">
            ...
            <input type="text" bind:value={form.website} class:filled={form.website.length > 3} placeholder="Website" />
            ...
            <h6>{err}</h6>
            <button class="button colorRotate" on:click={sendContactMessage} on:mouseenter={zoomIn} on:mouseleave={zoomOut}>Submit</button>
        </div>
    </form>
    {:else}
